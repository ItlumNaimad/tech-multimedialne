...
CREATE TABLE IF NOT EXISTS `test` (
  ...
  `prog_zaliczenia` int(11) NOT NULL DEFAULT 50,
  PRIMARY KEY (`idt`),
...
CREATE TABLE IF NOT EXISTS `lekcje` (
  ...
  `plik_pdf` varchar(255) DEFAULT NULL,
  `kolejnosc` int(11) NOT NULL DEFAULT 0,
  PRIMARY KEY (`idl`),
  FOREIGN KEY (`idc`) REFERENCES `coach`(`idc`) ON DELETE CASCADE,
  FOREIGN KEY (`idt`) REFERENCES `test`(`idt`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_polish_ci;
...
CREATE TABLE IF NOT EXISTS `pytania` (
  ...
  `typ_wyboru` enum('single','multi') NOT NULL DEFAULT 'single',
  `plik_multimedialny` varchar(255) DEFAULT NULL,
  PRIMARY KEY (`idpyt`),
  FOREIGN KEY (`idt`) REFERENCES `test`(`idt`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_polish_ci;

-- Tabela odpowiedzi do pytań
CREATE TABLE IF NOT EXISTS `odpowiedzi` (
  `idodp` int(11) NOT NULL AUTO_INCREMENT,
  `idpyt` int(11) NOT NULL,
  `tresc` varchar(255) NOT NULL,
  `czy_poprawna` tinyint(1) NOT NULL DEFAULT 0,
  PRIMARY KEY (`idodp`),
  FOREIGN KEY (`idpyt`) REFERENCES `pytania`(`idpyt`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_polish_ci;
