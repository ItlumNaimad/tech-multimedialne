 ...
 * Cel: Sprawdzenie testu, generowanie PDF (Stabilna obsługa polskich znaków).
 */
session_start();
...
if ($has_fpdf) {
    try {
        ...
        // REJESTRACJA POPRAWNEJ CZCIONKI POLSKIEJ
        $pdf->AddFont('Polski', '', 'polski.php');
        $pdf->AddFont('Polski', 'B', 'polskib.php');
        
        $pdf->AddPage();
        $pdf->SetFont('Polski', 'B', 16);
        $pdf->Cell(0, 10, iconv('UTF-8', 'ISO-8859-2', "RAPORT Z TESTU: " . $test['nazwa']), 0, 1, 'C');
        $pdf->Ln(5);

        $pdf->SetFont('Polski', '', 11);
        $pdf->Cell(0, 7, iconv('UTF-8', 'ISO-8859-2', "Użytkownik: " . $_SESSION['username']), 0, 1);
        $pdf->Cell(0, 7, iconv('UTF-8', 'ISO-8859-2', "Data: " . date("Y-m-d H:i:s")), 0, 1);
        $pdf->Cell(0, 7, iconv('UTF-8', 'ISO-8859-2', "Wynik: $total_points / " . count($questions)), 0, 1);
        $pdf->Cell(0, 7, iconv('UTF-8', 'ISO-8859-2', "Procentowo: " . round($percent) . "%"), 0, 1);
        
        $pdf->Ln(5);
        if ($passed) {
            $pdf->SetTextColor(0, 120, 0);
            $pdf->SetFont('Polski', 'B', 12);
            $pdf->Cell(0, 10, iconv('UTF-8', 'ISO-8859-2', "STATUS: ZALICZONY"), 0, 1);
        } else {
            $pdf->SetTextColor(200, 0, 0);
            $pdf->SetFont('Polski', 'B', 12);
            $pdf->Cell(0, 10, iconv('UTF-8', 'ISO-8859-2', "STATUS: NIEZALICZONY"), 0, 1);
        }
